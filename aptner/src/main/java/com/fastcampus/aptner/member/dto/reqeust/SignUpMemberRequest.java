...
public class SignUpMemberRequest {
    ...
    private boolean snsMarketingInformationReceive; // SNS 마케팅 정보 수신 동의는 선택입니다.
    ...
    @Size(max = 6)
    private String birthFirst; // 본인인증없이 가입된 회원은 선택입니다.

    @Pattern(regexp = "^[1-4]$", message = "주민등록번호: 1자리 숫자를 입력해주세요.")
    @Size(max = 1)
    private String gender;  // 본인인증없이 가입된 회원은 선택입니다.

    @Pattern(regexp = "^[a-zA-Z가-힣]+$", message = "통신사: 통신사를 선택하세요.")
    @Size(min = 2, max = 10)
    private String phoneCarrier; // 본인인증없이 가입된 회원은 선택입니다.
